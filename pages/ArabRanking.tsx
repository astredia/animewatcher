...
import { fetchArabTopRated, watchlistService } from '../services/api';
import { Anime } from '../types';
import { Link } from 'react-router-dom';
import { Star, TrendingUp, Plus, Check, Play, Award } from 'lucide-react';
import { useNotification } from '../context/NotificationContext';
import { useAuth } from '../context/AuthContext';

export const ArabRanking = () => {
    ...
    useEffect(() => {
        const load = async () => {
            setLoading(true);
            const data = await fetchArabTopRated(); 
            setAnimes(data);
            setLoading(false);
            if (user) {
    ...
    const toggleWatchlist = async (e: React.MouseEvent, anime: Anime) => {
        ...
        if (!user) {
            showNotification('يرجى تسجيل الدخول للإضافة للقائمة', 'info');
            return;
        }
    ...
    return (
        <div className="pt-24 min-h-screen bg-background container mx-auto px-4 md:px-12 pb-12" dir="rtl">
            <div className="flex flex-col md:flex-row items-center justify-between mb-8 border-b border-white/10 pb-6">
                <h1 className="text-3xl font-black text-white flex items-center gap-3">
                   <Award className="text-yellow-500" fill="currentColor"/> الترتيب العربي
                </h1>
                <p className="text-gray-400 mt-2 md:mt-0">أفضل الأعمال تقييماً لدى الجمهور العربي</p>
            </div>
                        ...
                        <div key={anime.id} className="group bg-surface hover:bg-[#222] border border-white/5 rounded-xl p-4 flex gap-6 transition-all duration-300 hover:scale-[1.01] hover:shadow-xl hover:border-white/10">
                            ...
                            <div className="flex-1 py-2 text-right">
                                ...
                                <div className="flex items-center gap-4 text-sm text-gray-400 mb-3">
                                    ...
                                    <div>{anime.type}</div>
                                    <div>{anime.totalEpisodes ? `${anime.totalEpisodes} حلقة` : 'مستمر'}</div>
